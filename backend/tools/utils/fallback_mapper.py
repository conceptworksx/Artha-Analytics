...
def map_fallback_to_yf(data: dict, original_ticker: str) -> dict:
    ...
    try:
        ...
        mapped_info = {
            ...
            "trailingPE": safe_float(info_dict.get("pPerEBasicExcludingExtraordinaryItemsTTM")),
            "pegRatio": safe_float(info_dict.get("pegRatio")),
            "dividendYield": safe_float(info_dict.get("dividendYieldIndicatedAnnualDividendDividedByClosingprice")) / 100.0 if safe_float(info_dict.get("dividendYieldIndicatedAnnualDividendDividedByClosingprice")) is not None else None,
            "marketCap": None,  # Can add if available in peerCompanyList or similar
            "enterpriseToEbitda": safe_float(info_dict.get("currentEVPerFreeCashFlowLTM")),  # Close approximation or leave None if strictly EBITDA
        }
        ...
        if market_cap_crores is not None:
            mapped_info["marketCap"] = safe_float(market_cap_crores) * 10000000  # API provides in crores (1 crore = 10,000,000)
        else:
            ...
            if peer_list and isinstance(peer_list, list):
                for peer in peer_list:
                    if peer.get("companyName") == data.get("companyName"):
                        mapped_info["marketCap"] = safe_float(peer.get("marketCap")) * 10000000
                        break
        ...
        def build_df(records):
            ...
            df = df.pivot_table(index="Metric", columns="Date", values="Value", aggfunc="first")
